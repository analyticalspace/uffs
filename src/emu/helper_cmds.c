...
static int cmd_export_file(int argc, char * argv[])
{
    CHK_ARGC(3, 3);
    ...
    if ((host_fd = fopen(host_name, "w+")) == NULL)
    {
        MSGLN("Can't open host file %s", host_name);
        perror("fopen");
        goto fail;
    }
    ...
    while (uffs_eof(fd) == 0)
    {
        ...
        if (len == 0)
            break;
        if (len > 0) {
            fwrite(buf, 1, len, host_fd);
        }
    }

    MSGLN("Wrote from %s to %s", name, host_name);
    MSG(TENDSTR);
...
static int cmd_import_file(int argc, char * argv[])
{
    CHK_ARGC(3, 3);
    ...
    if ((fd = uffs_open(name, UO_RDWR|UO_CREATE|UO_TRUNC)) < 0) {
        MSGLN("Can't open %s", name);
        goto fail;
    }

    if ((host_fd = fopen(host_name, "r")) == NULL)
    {
        MSGLN("Can't open host file %s", host_name);
        perror("fopen");
        goto fail;
    ...
    do
    {
        len = fread(buf, 1, sizeof(buf), host_fd);

        if (len)
            uffs_write(fd, buf, len);

        if (feof(host_fd) && ferror(host_fd))
            break;
...
static int cmd_hexdump(int argc, char * argv[])
{
    ...
    uffs_stat(name, &stat_buf);
    max_len = stat_buf.st_size;

    //fprintf(stdout, "\e[1;30m");
    ascii[16] = '\0';
    ...
    while (uffs_eof(fd) == 0)
    {
        ...
        for (i = 0; i < len; ++i, ++j)
        {
            ...
            if (((unsigned char*)buf)[i] >= ' ' &&
                ((unsigned char*)buf)[i] <= '~')
            {
                ascii[j % 16] = ((unsigned char*)buf)[i];
            }
            else {
                ascii[j % 16] = '.';
            }

            if ((j + 1) % 8 == 0 || j + 1 == max_len)
            {
                fprintf(stdout, " ");
...
static const struct cli_command helper_cmds[] =
{
    ...
    { cmd_export_file , "export-file" , "<uffs file> <host file>" , "Exports a uffs file to host disk"                },
    { cmd_import_file , "import-file" , "<host file> <uffs file>" , "Imports a file from host disk to UFFS"           },
    { cmd_hexdump     , "hexdump|hd"  , "<path>"                  , "Hexdump entire file"                             },
